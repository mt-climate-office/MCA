...
### Temperature
@tbl-temp-trends shows the decadal rate of change from 1950-2015 for average annual temperatures across `r params$location`'s `r nrow(shp)` `r params$attr_plural`. We provide that rate of change both annually and by season for `r params$location`. We also present the average annual and average seasonal changes across the region. To account partially for autocorrelation we considered trends as significant with a conservative p value at p<0.05. 

```{r echo=FALSE}
#| label: tbl-temp-trends
#| tbl-cap: "Decadal rate of change for annual average temperatures in °F (°C) for each region in the study area from 1950-2015. A value of 0 indicates no statistically significant change between decadal averages."
...
by_state <- dplyr::full_join(
  normals::make_seasonal_trend_table(monthly_tmp, whole, "region", "mean", 1951, 2015),
  normals::make_seasonal_trend_table(monthly_tmp, whole, "region", "mean", 1951, 2015, TRUE),
  by="region"
) %>%
  dplyr::mutate(Division="Statewide")

state_tmmx_trend <- 
  normals::make_seasonal_trend_table(monthly_tmmx, whole, "region", "mean", 1951, 2015, TRUE) %$%
  round(Annual, 2) 

state_tmmn_trend <- 
  normals::make_seasonal_trend_table(monthly_tmmn, whole, "region", "mean", 1951, 2015, TRUE) %$%
  round(Annual, 2)
...
The rate of temperature change across `r params$location` was `r dplyr::filter(all_trends, Division == "Statewide")  %$% round(Annual, 2)`°F/decade (@tbl-temp-trends). Across `r params$attr_plural` average annual minimum and maximum temperature changes ranged from `r state_tmmx_trend`-`r state_tmmn_trend`°F/decade, respectively. Between 1950 and 2015, `r params$location`’s average annual temperature has `r get_trend_direction(state_tavg_trend)` by `r dplyr::filter(all_trends, Division == "Statewide") %$% round(Annual * 6.4, 2)`°F; annual maximum and minimum temperatures have `r get_trend_direction(state_tmmx_trend)` by approximately `r round(state_tmmx_trend * 6.4, 2)`°F and `r round(state_tmmn_trend * 6.4, 2)`°F, respectively.
...
### Precipitation
@tbl-ppt-trends shows the decadal rate of change from 1950-2015 for average total precipitation across `r params$location`'s `r nrow(shp)` `r params$attr_plural`. We provide that rate of change both annually and by season for `r params$location`. We also present the average annual and average seasonal changes across the region. To account partially for autocorrelation we considered trends as significant with a conservative p-value at p<0.05. 
...
by_div <- dplyr::full_join(
  ...
  normals::make_seasonal_trend_table(monthly_ppt, shp, "Division", "sum", 1951, 2015, TRUE),
  by="Division"
)

by_state <- dplyr::full_join(
  normals::make_seasonal_trend_table(monthly_ppt, whole, "region", "sum", 1951, 2015),
  normals::make_seasonal_trend_table(monthly_ppt, whole, "region", "sum", 1951, 2015, TRUE),
  by="region"
) %>%
  dplyr::mutate(Division="Statewide")
...
The rate of precipitation change across `r params$location` was `r dplyr::filter(ppt_trends, Division == "Statewide") %$% round(Annual, 2)`in./decade (@tbl-ppt-trends). Between 1950 and 2015, `r params$location`’s average annual precipitation has `r get_trend_direction(state_ppt_trend)` by `r dplyr::filter(ppt_trends, Division == "Statewide") %$% round(Annual * 6.4, 2)` inches.

### Extreme aspects of Montana’s climate **IN PROGRESS**
Along with analyzing historical trends in temperature and precipitation, we performed an analysis of changes in extreme climate events since the middle of last century. Two examples of climate extremes include periods of intense warm or cool temperatures and significant wet or dry spells across seasons. Because these events affect every aspect of our society, decision makers and stakeholders are increasingly in need of historical evaluations of extreme events and how they are changing from seasons to centuries. The coldest temperature ever observed in the conterminous US was -70°F at Rogers Pass outside of Helena on January 20, 1954. Since 1950, however, our analysis shows the average winter temperature has increased by **0.4°F/decade** across the state, with an overall average winter temperature increase of **3.6°F**. Average spring temperatures have increased by **2.6°F** during the same period, and average summer temperatures have risen by **2.0°F**. Montana’s fall average temperatures have increased by **1.6°F** since 1951.
